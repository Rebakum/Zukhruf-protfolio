    ...
    <section className="py-12 border-t border-slate-100 bg-white">
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Link
            to="/portfolio"
            className="inline-flex items-center gap-1.5 bg-[#E10F0F] hover:bg-[#C90D0D] text-white text-[13px] font-extrabold py-2.5 px-5 rounded-lg transition-colors duration-200 uppercase tracking-wider shadow-sm shadow-red-500/10"
          >
            View All Projects
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 justify-center items-center gap-6">
          {featured.map((item, index) => (
            <div
              key={index}
              className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-lg hover:shadow-red-200/60 hover:border-red-500 transition-all duration-300 group flex flex-col justify-between"
            >
              {/* Visual Logo Stage */}
              ...
              <div className="p-5 text-center space-y-3">
                <div className="space-y-1">
                  <h3 className="text-[18px] md:text-[20px] font-black text-slate-900 font-display">
                    {item.title}
                  </h3>
